...
class Model:
    def __init__(self) -> None:
        ...
        self.state = init_ks([self.x_pos, self.y_pos, self.delta, self.speed, self.heading_angle])  # 初始状态
        self.t_step = 0.05  # 减小时间步长以提高精度
        self.u = [0.0, 0.0]  # 初始化控制输入
    ...
    def fmi2DoStep(self, current_time, step_size, no_step_prior):
        ...
        # 为每个状态变量添加噪音
        noise = np.array([
            np.random.normal(0, self.noise_std['x_pos']),
            np.random.normal(0, self.noise_std['y_pos']),
            np.random.normal(0, self.noise_std['delta']),
            np.random.normal(0, self.noise_std['speed']),
            np.random.normal(0, self.noise_std['heading_angle'])
        ])
        self.state += noise  # 在状态中加入噪音
